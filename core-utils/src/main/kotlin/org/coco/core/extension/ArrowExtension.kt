...
inline fun <E, A> logic(
    @BuilderInference noinline block: Logic<E, A>,
): Logic<E, A> = block

@Suppress("NOTHING_TO_INLINE")
@OptIn(ExperimentalTypeInference::class)
inline fun <E, A> suspendLogic(
    @BuilderInference noinline block: SuspendLogic<E, A>,
): SuspendLogic<E, A> = block
...
context(Raise<E2>)
suspend fun <E1, E2, A> SuspendLogic<E1, A>.bindOrRaise(transform: (E1) -> E2): A = fold({ raise(transform(it)) }, ::identity)

@Suppress("UnusedReceiverParameter")
fun <T> Nothing?.toOption(): Option<T> = Option.fromNullable(null)

inline fun <T, R> Option<T>.bindOrNull(f: (T) -> R): R? = fold({ null }, f)
